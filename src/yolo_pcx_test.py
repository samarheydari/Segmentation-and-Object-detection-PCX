...
def plot_pcx_explanations(class_id, model_name, model, img, orig_img, dataset, orig_dataset,
    ...
    scores_det = scores_det[0]  # [N, C]
    boxes_det = boxes_det[0]  # [N, 4]
    ...
    print(f"[DBG] class(draw)={int(scores_det[prediction_num].argmax())}  "
          f"class(attr)={int(scores_attr[mapped[prediction_num]].argmax())}")
    ...
    box_letterbox = boxes_raw[prediction_num].clone().detach().float()[None]
